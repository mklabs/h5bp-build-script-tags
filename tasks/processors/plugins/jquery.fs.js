...
module.exports = function($, em, options) {

  em.emit('log', 'Init jquery.fs module');

  options = options || {};

  em.emit('data', options);
  ...
  return {
    ...
    concat: function concat(to, cb) {
      ...
      em.emit('log', 'Concat ' + ln + ' file' + (ln > 1 ? 's' : ''));
      ...
      return this.each(function(i, val) {
        ...
        em.emit('log', 'Concat file » ' + src + ' to ' + path.resolve(to));

        if(ln--) return;

        em.emit('log', 'Write concat files to ' + dest);
    ...
    minify: function minify(to, cb) {
      ...
      em.emit('log', 'Concat/Min ' + ln + ' file' + (ln > 1 ? 's' : ''));
      ...
      return this.each(function() {
        ...
        em.emit('log', 'Minify file » ' + src);

        if(ln--) return;

        em.emit('log', 'Write minified files to ' + dest);
        path.exists(dirname, function(exists) {
          if(exists) return fs.writeFile(dest, out.join('\n\n'), cb);
